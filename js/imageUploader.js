...
$(() => {
    ...
    $('.imageUploader').each(function () {
        ...
        $(this).append('<input  id="' + controlId + '_ImageUploader" type="file" class="uploadFile" style="visibility:hidden;height:0px;" ' +
            ' accept="' + acceptedFileFormat + '"> ');

        $(this).append('<input style="visibility:hidden;height:0px;" id="' + controlId + '" name="' +
            controlId + '" waitingImage ="' + waitingImage + '">');
...
function preLoadImage(event) {
    ...
    if (ImageUploader.value.length > maxImageSize) {
        alert(tooBigFileErrorMessage);
        UploadedImage.src = UploadedImage.getAttribute("previousImage");;
    }
    else {
